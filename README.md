...
7. Interrupt & resume
  ...
  - There is only one way to make a 'safe' interruption which is to tap `qm` quickly. This will signal the scanner to skip the rest of its queue and after all remaining downloads get processed the app will exit normally. Use `dm` sequence to also cancel all pending downloads, or `cm` to cause interruption of all active writes and immediate exit _(unfinished files may get deleted, according to current unfinished files policy)_
