...
from defs import (
    APP_NAME, APP_VERSION, DOWNLOAD_MODE_TOUCH,
)
...
class CmdTests(TestCase):
    ...
    def test_cmd_pages(self):
        ...
        parsed2 = prepare_arglist_pages(['-start', '2', '-pages', '1', '-uploader', 'user1', '(a~b)', '-script',
                                         'a: a; b: b; c: c -d; d: * -uvp always', '-naming', 'prefix|quality', '-log', 'warn'])
        c2 = BaseConfig()
        ...
        self.assertEqual('user1', c2.uploader)
    ...
    def test_cmd_ids(self):
        ...
        parsed2 = prepare_arglist_ids(['-start', '1000', '-end', '999', '(a~b)', '(c~d)', '-dmode', 'touch', '-script',
                                       'a: a; b: b; c: c -d; d: * -uvp always', '-naming', '0x10', '-log', 'trace'])
        c2 = BaseConfig()
