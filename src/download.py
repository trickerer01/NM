...
async def download_id(idi: int, my_title: str, my_rating: str, scenario: Optional[DownloadScenario]) -> DownloadResult:
    sname = f'{prefixp()}{idi:d}.mp4'
    my_subfolder = ''
...
async def download_file(idi: int, filename: str, my_dest_base: str, link: str, subfolder='') -> DownloadResult:
    ...
    if not path.isdir(my_dest_base):
        ...
        if file_exists_in_folder(my_dest_base, idi, nm_quality, len(subfolder) > 0):
