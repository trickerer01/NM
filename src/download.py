...
async def process_video(vi: VideoInfo) -> DownloadResult:
    ...
    for i in range(QUALITIES.index(vi.quality), len(QUALITIES)):
        vi.link = f'{SITE}/media/videos/{QUALITY_STARTS[i]}{vi.id:d}{QUALITY_ENDS[i]}{fname_part2}'
...
async def download_video(vi: VideoInfo) -> DownloadResult:
    ...
    if skip is True:
        ...
        if not path.isdir(vi.my_folder):
            ...
            nm_match = re_media_filename.match(vi.filename)
            nm_quality = nm_match.group(2)
            nm_curfile = file_already_exists(vi.id, nm_quality)
            if nm_curfile:
